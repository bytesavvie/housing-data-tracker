  ...
  try {
    ...
    for (let i = 1; i < csvArray.length - 2; i++) {
      ...
      if (row) {
        ...
        if (stateId && stateHash[stateId] && zipcode) {
          stateHash[stateId]?.push({ name: zipcode, value: zipcode });
        } else if (stateId && zipcode) {
          stateHash[stateId] = [{ name: zipcode, value: zipcode }];
        }
      }
    }
